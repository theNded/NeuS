...
def save_poses(realdir):
    ...
    ids = [k for k in imdata]

    print( 'Images #', len(names))
    perm = np.argsort(names)

    names = np.array(names)[perm]
    ids = np.array(ids)[perm]
    ...
    np.save(os.path.join(realdir, 'poses.npy'), poses)
    np.save(os.path.join(realdir, 'pose2fname.npy'), names)
